                      ...
                      int nTries, int timeout, const char *filename)
{
    ...
    else if (strcmp(role, "rx") == 0)
    {
        ...
        int packetSize = llread(packet); //verificar o valor do packetSize pois pode não estar a ser alterado
        if(packetSize == -1){
        ...
        unsigned char* name = parseControlPacket(packet, packetSize, &rxFileSize); //função a ser implementada e trocar um bocado a variaveis e assim (plagio)
        ...
        while (1) {
            ...
            if(packetSize == 0) break;
                ...
                parseDataPacket(packet, packetSize, buffer); //função a ser implementada e trocar um bocado a variaveis e assim (plagio)
